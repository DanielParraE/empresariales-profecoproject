...
public class PersonLdapService implements PersonRepository {
    ...
    private Attributes buildAttributes(Person p) {
        ...
        attrs.put("profecoId", p.getProfecoId());
    ...
    public Name buildDn(String userId) {
        return LdapNameBuilder.newInstance(BASE_DN).add("ou", "personal").add("uid", userId).build();
    }
    public Name buildBaseDn() {
        return LdapNameBuilder.newInstance(BASE_DN).add("ou", "personal").build();
    }
    @Override
    public List<Person> findAll() {
        SearchControls searchControls = new SearchControls();
        searchControls.setSearchScope(SearchControls.SUBTREE_SCOPE);
        List<Person> people = ldapTemplate.search(query().where("objectclass").is("person").and("profecoId").isPresent(),
    ...
    public Person findByUID(String uid) {
        SearchControls searchControls = new SearchControls();
        searchControls.setSearchScope(SearchControls.SUBTREE_SCOPE);
        List<Person> people = ldapTemplate.search(query().where("objectclass").is("person").and("profecoId").isPresent().and("uid").is(uid),
                new PersonAttributeMapper());
    ...
    private class PersonAttributeMapper implements AttributesMapper<Person> {
        ...
        public Person mapFromAttributes(Attributes attributes) throws NamingException {
            ...
            person.setProfecoId(
                    null != attributes.get("profecoId") ? attributes.get("profecoId").get().toString() : null);
